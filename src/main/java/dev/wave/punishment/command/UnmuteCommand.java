...
public class UnmuteCommand extends AbstractCommand {
    public UnmuteCommand() {
        super("unmute");
        setPermission("punishments.unmute");
    ...
    public void perform(CommandContext commandContext) {
        ...
        if(unbanned){
            Messages.UNMUTE_SUCCESS.send(commandContext.getCommandSender(), new Placeholder("{target}", target.getName()));
        }else{
            Messages.UNMUTE_FAILURE.send(commandContext.getCommandSender(), new Placeholder("{target}", target.getName()));
        }
